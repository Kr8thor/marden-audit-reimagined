...
import kvStore from '../storage/kv-store.mjs';
import processJob from '../../workers/index.mjs';
...
export async function processNextJob() {
  try {
    const jobIds = await jobQueue.getNextBatch();
    ...
    for (const jobId of jobIds) {
      try {
        ...
        if (job) {
          ...
          await jobQueue.completeJob(jobId, results);
          logger.info(`Completed job ${jobId}`, { jobId });
        }
      } catch (error) {
